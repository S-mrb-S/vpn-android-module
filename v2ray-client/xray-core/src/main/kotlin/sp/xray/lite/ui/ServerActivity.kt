...
import sp.xray.lite.AppConfig.WIREGUARD_LOCAL_ADDRESS_V6
import sp.xray.lite.AppConfig.WIREGUARD_LOCAL_MTU
import sp.xray.lite.R
import sp.xray.lite.dto.EConfigType
import sp.xray.lite.dto.ServerConfig
import sp.xray.lite.dto.V2rayConfig
import sp.xray.lite.dto.V2rayConfig.Companion.DEFAULT_PORT
import sp.xray.lite.dto.V2rayConfig.Companion.TLS
import sp.xray.lite.extension.removeWhiteSpace
import sp.xray.lite.extension.toast
import sp.xray.lite.util.MmkvManager
import sp.xray.lite.util.MmkvManager.ID_MAIN
import sp.xray.lite.util.MmkvManager.KEY_SELECTED_SERVER
import sp.xray.lite.util.Utils
import sp.xray.lite.util.Utils.getIpv6Address
...
class ServerActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        when (config?.configType ?: createConfigType) {
            EConfigType.VMESS -> setContentView(R.layout.activity_x_server_vmess)
            EConfigType.CUSTOM -> return
            EConfigType.SHADOWSOCKS -> setContentView(R.layout.activity_x_server_shadowsocks)
            EConfigType.SOCKS -> setContentView(R.layout.activity_x_server_socks)
            EConfigType.VLESS -> setContentView(R.layout.activity_x_server_vless)
            EConfigType.TROJAN -> setContentView(R.layout.activity_x_server_trojan)
            EConfigType.WIREGUARD -> setContentView(R.layout.activity_x_server_wireguard)
        }
    ...
    override fun onCreateOptionsMenu(menu: Menu): Boolean {
        menuInflater.inflate(R.menu.action_x_server, menu)
